...
import { CopyIcon, ExitIcon, CheckCircleIcon, BaseButton } from '@/components/common';
...
export type ConnectViewRequestingConnectionStatus =
  | 'requesting'
  | 'rejected'
  | 'error'
  | 'connected';

export interface ConnectViewRequestingConnectionProps {
  size?: 'sm' | 'md';
  status?: ConnectViewRequestingConnectionStatus;
  walletName: string;
...
const titleByStatus: Record<ConnectViewRequestingConnectionStatus, string> = {
...
const descByStatus: Record<ConnectViewRequestingConnectionStatus, string> = {
  requesting: 'Requesting connection',
...
export function ConnectViewRequestingConnection(props: ConnectViewRequestingConnectionProps) {
  ...
  return (
    ...
    >
      {props.status === 'requesting' || props.status === 'rejected' ? (
      ...
      <Box>
        {size === 'sm' && props.status === 'connected' ? null : (
          <Box
      ...
      >
        ...
        {props.status === 'connected' && size === 'md' && (
          <Text as="span" fontSize="$3xl" color="inherit">
        ...
        {props.status !== 'connected' && size === 'md' && (
          <Text as="span" color="$text" fontSize="$xs" fontWeight="$bold">
            {titleByStatus[props.status ?? 'requesting']}
          </Text>
        )}

        {props.status === 'connected' && size === 'sm' && (
          <Box
            width="100%"
            display="flex"
            gap="$4"
          ...
          >
            ...
            {!!props.walletAddress && (
              <>
                ...
                >
                  {shortenAddress(props.walletAddress, 5)}
                </Text>
                ...
                >
                  <AnimatePresence mode="wait">
                    {showSuccessIcon ? (
                      ...
                      >
                        <Box as="span" fontSize="18px" color="$textSuccess">
                          <CheckCircleIcon />
                        </Box>
                      </motion.div>
                    ) : (
            ...
            <BaseButton aria-label="Exit" onPress={() => props.onDisconnect?.()}>
              <Box height="16px">
                <ExitIcon />
              </Box>
            </BaseButton>
      ...
      <Text as="p" color="$text" fontSize="$xs" fontWeight="$bold">
